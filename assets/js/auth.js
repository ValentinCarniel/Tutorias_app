...
document
  ...
  .addEventListener("submit", async function (e) {
    ...
    try {
      const response = await fetch("http://localhost/Tutoria/api/login.php", {
        method: "POST",
      ...
      if (response.ok && data.success) {
        ...
        const destino =
          data.rol === "tutor" ? "tutor_panel.html" : "Tutorias.html";
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        Swal.fire({
          ...
          const authModalInstancia = bootstrap.Modal.getInstance(
            document.getElementById("authModal")
          );
          if (authModalInstancia) {
            authModalInstancia.show();
          } else {
            const nuevoAuthModal = new bootstrap.Modal(
              document.getElementById("authModal")
            );
            nuevoAuthModal.show();
          }
